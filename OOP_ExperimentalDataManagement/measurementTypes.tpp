...
template <class T> std::shared_ptr<measuremnt<T>> numMeas<T>::operator*(const std::shared_ptr<measuremnt<T>> m) const {//override multiplication with error propagation
	std::string outName{ name_ + "*" + m->getName() };
	T outMeas{ meas_*m->getMeas() };
	T outMeasErr{ (T)(sqrt(pow(measErr_/meas_,2) + pow(m->getMeasErr()/m->getMeas(),2)) * outMeas) };
	T outSysErr{ (T)(meas_*m->getMeasErr() + m->getMeas()*measErr_ + m->getMeasErr()*measErr_) };
	std::shared_ptr<measuremnt<T>> out(new numMeas(outName, outMeas, outMeasErr, outSysErr, 0));
	return out;
}
...
template <class T> std::shared_ptr<measuremnt<T>> numMeas<T>::operator/(const std::shared_ptr<measuremnt<T>> m) const {//override division with error propagation
	...
	T outMeas{ meas_/m->getMeas() };
	T outMeasErr{ (T)(sqrt(pow(measErr_ / meas_,2) + pow(m->getMeasErr() / m->getMeas(),2)) * outMeas) };
	T outSysErr{ (T)((m->getMeas()*measErr_ - meas_*(m->getMeasErr())) / (m->getMeas()*(m->getMeas() + m->getMeasErr()))) };
	std::shared_ptr<measuremnt<T>> out(new numMeas(outName, outMeas, outMeasErr, outSysErr, 0));
	return out;
}
...
template<class T> std::shared_ptr<measuremnt<T>> numMeas<T>::operator/(const double& d) const {//override division by constant
	...
	outNameSS << name_ << "/" << d;
	T outMeas{ (T)(meas_ / d) };
	T outMeasErr{ (T)(measErr_ / d) };
	T outSysErr{ (T)(sysErr_ / d) };
